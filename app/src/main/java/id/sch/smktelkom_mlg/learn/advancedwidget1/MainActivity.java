...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button bProsses = new Button(this);
        bProsses.setText("Proses");
        llMain.addView(bProsses);
        final TextView tvHasil = new TextView(this);
        llMain.addView(tvHasil);

        bProsses.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void doProses(LinearLayout llMain) {
        ...
        for (int i = 0; i <= 5; i++) {
            ...
            if (umur.isEmpty())
                umur = "0";
            if (!nama.isEmpty())
                hasil += "Anak ke-" + (1 + i) + ": " + nama + "umur" + umur + "tahun\n";
        }
